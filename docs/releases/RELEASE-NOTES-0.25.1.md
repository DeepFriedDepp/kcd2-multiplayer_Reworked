...
| pause save-persistence | (code-verified) `C_IntelligentObject::Save` DOES write the suspend byte to a save chunk, conditionally; **(observed)** live quicksave+reload confirmed the load side does NOT re-apply it — for the same machine's own reload. A different machine's reload disturbing an actively-paused NPC here is a genuinely different question and remains untested; solo can't produce it (findings §1.3.2) |
