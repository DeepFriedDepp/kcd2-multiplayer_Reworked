...
usage: python start_joiner.py <KcdMpClient.dll> [KcdMpServer.dll]
...
if not listening(7778):
    ...
    subprocess.Popen([live.dotnet(), relay, '--port', '7778'], cwd=os.path.dirname(relay),
...
time.sleep(2.5)
a = subprocess.Popen([live.dotnet(), agent, '--host', '127.0.0.1', '--port', '7778', '--name', 'wo118-joiner', '--no-voice', '--no-discord'],
