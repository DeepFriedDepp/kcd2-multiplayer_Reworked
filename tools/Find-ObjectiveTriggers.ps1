...
# WO-97 Phase 0. "clean" (no extra DIRECT effect) is not "safe": the state
# transition itself pulses its On<State> consumers, and for these five the
# transitive pulse chain -- across Output ports into parent modules --
# reaches a CutsceneHandler.EnqueueCutscene (code-verified, docs/WO-97-findings.md s1).
# Cutscene start-time is a confirmed hard engine limit with no pre-fire
# warning, so a fix that starts one is a worse bug wearing a fix's clothes.
# Keyed on the trigger path so a regeneration cannot silently re-add them.
$hazardCutscene = @{
    'vezniNaTroskach.startApolenaGameplay'          = 'OnDone -> cin_m1280t_vezninatroskach__zikmund_letter.enqueue_cs -> CutsceneHandler.EnqueueCutscene'
    'sedmStatecnych.sedmStatecnych_kubenkaZachranen' = 'OnDone -> prijezd_do_certovky.trialog_s_zizkou_a_kubenkou.start_trialog -> CutsceneHandler.EnqueueCutscene'
    'setkaniVRatbori2.pickWineSkip'                  = 'OnDone -> zacatek_na_party.spustit_cutscenu_utoku -> cin_m3780k_setkaniratbordva__ratbor_attack -> CutsceneHandler.EnqueueCutscene'
    'pogrom.04a_cutscene_blockadeFire'               = 'OnDone -> cin_m4220k_pogrom__blockade_fire.cutscena_utek_podzemim -> CutsceneHandler.EnqueueCutscene'
    'prepadeniVlasskehoDvora.init_end2'              = 'OnDone -> bohuta__vlassky_dvur -> fader_a_priprava_npc -> CutsceneHandler.EnqueueCutscene'
}
$fixes = @{}
$skippedReset = 0; $skippedTest = 0; $dupes = 0; $skippedHazard = 0
foreach ($r in ($rows | Where-Object { $_.clean -eq $true })) {
    if ($testRx.IsMatch($r.trigger)) { $skippedTest++; continue }
    if ($hazardCutscene.ContainsKey($r.trigger)) {
        $skippedHazard++
        Write-Host ("    WO-97 hazard (cutscene) skipped: {0} -- {1}" -f $r.trigger, $hazardCutscene[$r.trigger]) -ForegroundColor Yellow
        continue
    }
...
Write-Host ("  fixes      : {0} grant triggers for the mod ({1} reset ports skipped, {2} test/debug names skipped, {3} duplicates skipped, {4} WO-97 cutscene hazards skipped)" -f $fixList.Count, $skippedReset, $skippedTest, $dupes, $skippedHazard)
...
[void]$sb.AppendLine(("-- MINUS the {0} whose transitive On<State> pulse chain reaches a" -f $skippedHazard))
[void]$sb.AppendLine('-- CutsceneHandler.EnqueueCutscene (WO-97 Phase 0; KCD2MP_OBJECTIVE_FIX_HAZARDS).')
...
[void]$sb.AppendLine('-- reported as a gap only. See docs/WO-96-findings.md s4, docs/WO-97-findings.md s1.')
