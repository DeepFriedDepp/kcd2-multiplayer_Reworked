<#
.SYNOPSIS  WO-27 scratch driver: send a Lua chunk over ExecuteString, read [WO27] lines back.
.EXAMPLE   . tools\wo27-lua.ps1 ; Reset-Seen ; Lua 'W("hi")' ; Show
#>
...
function Show([int] $waitMs = 1200) {
    ...
    foreach ($line in (Get-Content $KcdLog)) {
        if ($line -notmatch '\[WO27\]') { continue }
        $clean = ($line -replace '.*\[WO27\] ', '')
...
function Reset-Seen {
    foreach ($line in (Get-Content $KcdLog)) {
        if ($line -match '\[WO27\]') { $script:seen[($line -replace '.*\[WO27\] ', '')] = $true }
    }
}
...
function Init-W {
    Lua 'function W(s) System.LogAlways("[WO27] " .. tostring(s)) end'
}
