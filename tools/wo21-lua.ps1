<#
.SYNOPSIS  WO-21 scratch driver: send a Lua chunk over ExecuteString, read [WO21] lines back.
.EXAMPLE   . tools\wo21-lua.ps1 ; Lua 'WO21P("hi",1)' ; Show
#>
...
function Show([int] $waitMs = 1200) {
    ...
    foreach ($line in (Get-Content $KcdLog)) {
        if ($line -notmatch '\[WO21\]') { continue }
        $clean = ($line -replace '.*\[WO21\] ', '')
...
function Reset-Seen {
    foreach ($line in (Get-Content $KcdLog)) {
        if ($line -match '\[WO21\]') { $script:seen[($line -replace '.*\[WO21\] ', '')] = $true }
    }
}
